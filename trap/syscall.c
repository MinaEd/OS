...
/*******************************/
uint32 sys_Zalabya(uint32* PTR_Table,uint32 va)
{
    return (PTR_Table[PTX(va)] & PERM_USED);
}
uint32* sys_get_page_table(uint32 va)
{
    uint32 * ay7aga=NULL;
  int ret =  get_page_table(curenv->env_page_directory,va,&ay7aga);
...
uint32 sys_get_break(){
	return curenv->uHeapBreak;
}
uint32 sys_get_hard_limit()
{
  return curenv->hardLimitOfUHeap;
}
//void sys_insert_list(struct mallocVaSize * Za3tar){
//
//     	if(((struct list_El_malloc)(curenv->list)).size == 0 ){
//     		LIST_INSERT_HEAD((&((struct list_El_malloc)curenv->list)),Za3tar);
//     	}
//     	else{
//     		LIST_INSERT_TAIL((&((struct list_El_malloc)curenv->list)),Za3tar);
//     	}
//}
...
uint32 sys_get_size(uint32 virtual_address){
	...
	struct FrameInfo * ptr_frame=get_frame_info(curenv->env_page_directory,virtual_address,&ptr_table);
	cprintf("the frame is %p \n",ptr_frame);
		return ptr_frame->size;
...
void* sys_sbrk(int increment)
{
	...
	if(increment==0){
		...
		if(increment > 0){
			uint32 newbreak = ROUNDUP((env->uHeapBreak+increment),PAGE_SIZE);
			if(newbreak > env->hardLimitOfUHeap){
				//panic("break exceeds hard limit");
				return (void*)-1 ;
			}
			uint32 oldbreak = env->uHeapBreak;
			env->uHeapBreak = newbreak;
			allocate_user_mem(env,oldbreak,ROUNDUP(increment,PAGE_SIZE));
			return (void *)oldbreak;
		}
		else if (increment < 0){
			uint32 newbreak  = env->uHeapBreak+increment;
		uint32 round = ROUNDUP(newbreak,PAGE_SIZE);
		uint32 rounddown = ROUNDDOWN(env->uHeapBreak,PAGE_SIZE);
		uint32 freesize = rounddown - round;
		free_user_mem(env,round,rounddown);
		env->uHeapBreak = newbreak;
			return (void *)env->uHeapBreak;
	}
		return (void*)-1 ;
...
//	case SYS_insert_list:
//		return sys_insert_list((struct mallocVaSize *)a1);
//		break;
	case SYS_get_size:
		return sys_get_size(a1);
		break;
	case SYS_get_break:
		return sys_get_break();
		break;
	case SYS_Zalabya:
	        return sys_Zalabya((uint32*)a1,a2);
	        break;
	case SYS_get_page_table:
	        return (uint32)sys_get_page_table(a1);
	        break;
