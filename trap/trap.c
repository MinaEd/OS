...
void fault_handler(struct Trapframe *tf)
{
	...
	else
	{
		if (userTrap)
		{
			...
			/*============================================================================================*/
			int pagePerm=pt_get_page_permissions(faulted_env->env_page_directory, fault_va);
			            if((fault_va >= USER_HEAP_START && fault_va < USER_HEAP_MAX && ((pagePerm & PERM_USED)==0)))
			            {
			                cprintf("1ST CONDITION\n");
			                sched_kill_env(faulted_env->env_id);
			            }
			            else if((pagePerm & PERM_PRESENT) && ((pagePerm & PERM_WRITEABLE)==0))
			            {
			                cprintf("READ only\n");
			                sched_kill_env(faulted_env->env_id);
			            }
		...
		else
		{
			...
			//cprintf("okkkkk in fault handler \n");
			page_fault_handler(faulted_env, fault_va);
		}
