...
inline struct WorkingSetElement* env_page_ws_list_create_element(struct Env* e, uint32 virtual_address)
{
	...
	if(element==NULL){
		panic("couldn't alloc");
	}
	uint32 * ptr_table=NULL;
	struct FrameInfo * ptr_frame = get_frame_info(e->env_page_directory,virtual_address,&ptr_table);
   ptr_frame->element = element;
	element->virtual_address=virtual_address;
	//cprintf("El create done,%u\n",element->virtual_address);
	return element;
}
